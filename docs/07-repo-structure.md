...
## TL;DR
**Start with ONE monorepo. Features are folders, not repos.** Multiple repos solve a *team coordination* problem you don't have yet, and they make cross-cutting changes (especially the shared action contract) painful for a solo founder. Organize with clean internal seams so you *can* split later — split only when it pays off.
